...
int main(int argc, char* argv[])
{
    ...
    for (int i=0; i<2; i++)
    {
        Ptr<ERFilter> er_filter1 = createERFilterNM1(loadClassifierNM1(std::string(OCR_RESOURCES_PATH)+std::string("trained_classifierNM1.xml")),8,0.00015f,0.13f,0.2f,true,0.1f);
        Ptr<ERFilter> er_filter2 = createERFilterNM2(loadClassifierNM2(std::string(OCR_RESOURCES_PATH)+std::string("trained_classifierNM2.xml")),0.5);
    ...
    Mat transition_p;
    string filename = std::string(OCR_RESOURCES_PATH) + std::string("OCRHMM_transitions_table.xml");
    ...
    for (int o=0; o<num_ocrs; o++)
    {
      decoders.push_back(OCRHMMDecoder::create(loadOCRHMMClassifierNM(std::string(OCR_RESOURCES_PATH)+ std::string("OCRHMM_knn_model_data.xml.gz")),
